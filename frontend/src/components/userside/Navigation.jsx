...
function Navigation() {
  ...
  let activeclase =
    "flex gap-3 py-3 bg-socialblue text-white   rounded-md shadow-md shadow-gray-300";
  let nonactiveclase =
  ...
  return (
    <div className={ismobile ? "hidden" : "  px-4 py-2  sticky top-20  "}>
      <Cards>
        ...
        <Link
          to={`/profile/friends/${logedinuser._id}`}
          className={
            location.pathname === `/profile/friends/${logedinuser._id}`
              ? activeclase
              : nonactiveclase
          }
        >
        ...
        <p
          onClick={logout}
          className="flex gap-2 py-3  hover:bg-blue-400 hover:bg-opacity-20  rounded-md  transition-all hover:scale-110 cursor-pointer"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
